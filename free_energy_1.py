...
# def compute_loss1(model, ob_txy, frozen_para, gamma = args.gamma, eps = args.eps):
#     output = vmap(net, (None, 0, None))(model, ob_txy[:, 0], frozen_para)
#
#     return jnp.sum(jnp.abs(ob_txy[:,1] - gamma * (ob_txy[:,2] + ob_txy[:,3] - output/(eps**2)))**2)/len(ob_txy)
...
def train(key):
    ...
    target_f = x_train[:,0] * (x_train[:,0] ** 2 - 1)
    history = []
    T = []
    for j in range(ite * N_epochs):
        ...
        if j % N_epochs == 0:
            ...
            target_f = x_train[:, 0] * (x_train[:, 0] ** 2 - 1)
            # train_mse_error = jnp.sum(jnp.abs(x_train[:,1] - gamma * (x_train[:,2] + x_train[:,3] - train_y_pred/(eps**2)))**2)/len(x_train)
            train_mse_error = jnp.sum(jnp.abs(train_y_pred - target_f) ** 2) / len(train_y_pred)
            train_relative_error = jnp.linalg.norm(train_y_pred - target_f) / jnp.linalg.norm(target_f)
