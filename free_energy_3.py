## This code is used to implement method 2 (algebraic-informed neural network) for learning the free energy of Allen-Cahn equation
import sys
...
def compute_loss(model, ob_txy, target_f, frozen_para):
    output = vmap(net, (None, 0, None))(model, ob_txy[:, 0], frozen_para)

    return jnp.sum(jnp.abs(output - target_f) ** 2) / len(output)

compute_loss_and_grads = eqx.filter_value_and_grad(compute_loss)

def make_step(model, ob_txy, target_f, frozen_para, optim, opt_state):
    loss, grads = compute_loss_and_grads(model, ob_txy, target_f, frozen_para)
...
def train(key):
    ...
    target_f = (eps ** 2) * (x_train[:, 2] + x_train[:, 3] - (1/gamma) * x_train[:, 1])
    history = []
    T = []
    for j in range(ite * N_epochs):
        T1 = time.time()
        loss, model, opt_state = make_step(model, x_train, target_f, frozen_para, optim, opt_state)
        T2 = time.time()
        T.append(T2 - T1)
        ...
        if j % N_epochs == 0:
            ...
            target_f = (eps ** 2) * (x_train[:, 2] + x_train[:, 3] - (1/gamma) * x_train[:, 1])
            ground_f = x_train[:, 0] * (x_train[:, 0] ** 2 - 1)
            data_to_ground = jnp.sum(jnp.abs(target_f - ground_f) ** 2) / len(target_f)
            train_mse_error = jnp.sum(jnp.abs(train_y_pred - ground_f) ** 2) / len(train_y_pred)
            train_relative_error = jnp.linalg.norm(train_y_pred - ground_f) / jnp.linalg.norm(ground_f)
            train_mse_error1 = jnp.sum(jnp.abs(train_y_pred - target_f) ** 2) / len(train_y_pred)
            train_relative_error1 = jnp.linalg.norm(train_y_pred - target_f) / jnp.linalg.norm(target_f)
            print(f'ite:{j},mse:{train_mse_error:.2e},relative mse:{train_relative_error:.2e},mse1:{train_mse_error1:.2e},relative mse1:{train_relative_error1:.2e},data to ground truth error:{data_to_ground:.2e}')
    ...
    np.savez(path, loss=history, avg_time=avg_time, y_pred=train_y_pred,
             mse=train_mse_error, relative_mse=train_relative_error, mse1=train_mse_error1, relative_mse1=train_relative_error1, data_to_ground=data_to_ground)

    # write the reuslts on csv file
    header = "network, seed, final_loss_mean, training_time, total_ite, mse, relative_mse, mse1, relative_mse1, data_to_ground"
    save_here = "results.csv"
    if not os.path.isfile(save_here):
        with open(save_here, "w") as f:
            f.write(header)

    res = f"\n{args.network},{args.seed},{history[-1]},{np.sum(np.array(T))},{ite * N_epochs},{train_mse_error},{train_relative_error},{train_mse_error},{train_relative_error1},{data_to_ground}"
